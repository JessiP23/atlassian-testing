...
test('a green gate goes on to deploy (then QA, then publish), always', () => {
  assert.equal(afterVerifyWith(clock(0))(green), 'deploy')
  assert.equal(afterVerifyWith(clock(19.9))(green), 'deploy', 'a green run must publish even at the deadline')
})
...
test('every route a predicate can return is a node the graph declares', () => {
  const declared = new Set(['intake', 'locate', 'planning', 'reproduce', 'patch', 'verify', 'repair', 'handover', 'deploy', 'browserqa', 'publish', 'refuse'])
  const returned = [
